...
if not favorites:
    st.info("You haven't added any favorite recipes yet. Go find some delicious meals! 🍳")
else:
    cols = st.columns(3)
    ...
    for index, recipe in enumerate(favorites):
        with cols[index % 3]:
            with st.container(border=True):
                ...
                with col_day:
                    selected_day = st.selectbox("Day", ["Day 1", "Day 2", "Day 3"], key=f"day_{recipe['recipe_id']}")
                with col_meal:
                    selected_meal = st.selectbox("Meal", ["Breakfast", "Lunch", "Dinner"], key=f"meal_{recipe['recipe_id']}")

                # Geçici hafıza anahtarı
                plan_state_key = f"plan_info_{recipe['recipe_id']}"

                if st.button("➕ Add to Plan", key=f"add_plan_{recipe['recipe_id']}", use_container_width=True):
                ...
                if plan_state_key in st.session_state:
                    ...
                    if missing_items:
                        ...
                        if st.button("🛒 Add Missing Items to Grocery List", key=f"cart_{recipe['recipe_id']}", use_container_width=True):
                ...
                if st.button("💔 Remove from Favorites", key=f"remove_{recipe['recipe_id']}", use_container_width=True):
                    payload = {
                        "email": EMAIL,
                        "recipe_id": recipe['recipe_id'],
                        "recipe_title": recipe['recipe_title']
