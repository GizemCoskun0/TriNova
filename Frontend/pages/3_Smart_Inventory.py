...
st.set_page_config(
    page_title="Smart Inventory", layout="wide", initial_sidebar_state="collapsed"
)
...
with col1:
    ...
    st.info(
        "Show your ingredients to the camera or upload a photo to detect them automatically."
    )
    ...
    with tab2:
        uploaded_picture = st.file_uploader(
            "Upload an image of your ingredients", type=["jpg", "jpeg", "png"]
        )
    ...
    if picture:
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get("status") == "success":
                            ...
                            if not detected_items:
                                st.warning(
                                    "🤷 AI couldn't detect any ingredients in this image."
                                )
                            else:
                                for item in detected_items:
                                    ...
                                    if res.get("status") != "success":
                                        st.error(
                                            f"⚠️ Error adding {item}: {res.get('message')}"
                                        )
        ...
        if st.session_state.ai_analyzed:
            ...
            if found_items:
                ...
                st.info(
                    "No items were detected. You can add them manually from the right side."
                )
...
with col2:
    ...
    with input_col1:
        st.text_input(
            "Ingredient:",
            placeholder="e.g., Asparagus, Milk",
    ...
    st.button(
        "➕ Add to My Kitchen", use_container_width=True, on_click=handle_add_item
    )
        ...
        for recipe in recipes:
            with st.expander(f"🍲 {recipe['title']}"):
                ...
                with col_info:
                    ...
                    st.write(
                        f"**Missing Ingredients:** {recipe['missedIngredientCount']}"
                    )
                    ...
                    if raw_instr:
                        ...
                        st.info(
                            "💡 Instructions are not provided for this recipe by the source."
                        )
