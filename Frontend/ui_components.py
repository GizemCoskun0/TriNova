...
@st.dialog("🍽️ Recipe Details", width="large")
def show_recipe_details_dialog(item, meal_plan_id, email, user_favorites=None):
    ...
    with col_modal_info:
        st.markdown(f"## {meal_emoji} {recipe_title}")
        st.markdown(f"**Category:** {meal_type}")
        st.markdown(
        ...
        if st.button(
            heart_icon, key=f"modal_fav_btn_{meal_plan_id}", use_container_width=True
        ):
            fav_payload = {
                "email": email,
                "recipe_id": recipe_id,
    ...
    if st.button(
        ...
        with st.spinner("Checking your inventory..."):
            try:
                check_response = requests.post(API_CHECK_INGREDIENTS, json=payload)
        ...
        with st.expander(f"🍲 {recipe['title']}"):
            ...
            with col_info:
                ...
                if category_key == "breakfast":
                    ...
                    meal_options = ["Soup"]
                elif category_key == "dessert":
                    meal_options = ["Dessert"]
                elif category_key == "main":
                    meal_options = ["Lunch", "Dinner", "Breakfast"]
        ...
        for index, item in enumerate(day_items):
            ...
            with cols[index % 7]:
                with st.container(border=True):
                    ...
                    if st.button(
                        "📄 View Details",
                        key=f"view_btn_{meal_plan_id}",
                        use_container_width=True,
                    ):
                        show_recipe_details_dialog(
                            item, meal_plan_id, email, user_favorites=user_favorites
                        )
