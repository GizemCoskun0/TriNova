...
async def get_recipes(
    ingredients: str, username: str, number: int = 5, db: Session = Depends(get_db)
):
    ...
    if user:
        user_diets = [
            CEVIRI.get(diet.name.lower(), diet.name.lower()) for diet in user.diets
        ]
        user_allergies = [
            CEVIRI.get(allergy.name.lower(), allergy.name.lower())
    ...
    if recipes:
        ...
        for recipe in recipes:
            ...
            for missing_ing in recipe.get("missedIngredients", []):
                ...
                is_actually_owned = any(
                    ing_name in owned or owned in ing_name for owned in all_ing_lower
                )
